...
def slow_stochastic(data, k_period, d_period):
...
def trading_simulation(individual):
    k_period, d_period, erc_tp, erc_sl = individual
    local_df = df.copy()
    local_df = slow_stochastic(local_df, k_period, d_period)
    
    trades = []
    position_open_price = []

    for i in range(1, len(local_df)):
        prev_k = local_df['%K'].iloc[i-2]
        prev_d = local_df['%D'].iloc[i-2]
        current_k = local_df['%K'].iloc[i-1]
        current_d = local_df['%D'].iloc[i-1]
        current_price = local_df['Close'].iloc[i]

        if prev_k <= prev_d and current_k > current_d and current_k <= 20 and len(position_open_price) < 3:
            position_open_price.append(current_price)
        
        new_positions_open_prices = []
        for price in position_open_price:
            trade_return = (current_price - price) / price
            if (prev_k >= prev_d and current_k < current_d and current_k >= 80) or \
               trade_return >= erc_tp / 100 or trade_return <= -erc_sl / 100:
                trades.append(trade_return)
            else:
                new_positions_open_prices.append(price)
        position_open_price = new_positions_open_prices
...
toolbox.register("attr_k_period", random.randint, 5, 30)
toolbox.register("attr_d_period", random.randint, 3, 14)
toolbox.register("attr_take_profit", generate_erc)
toolbox.register("attr_stop_loss", generate_erc)
toolbox.register("individual", tools.initCycle, creator.Individual, (toolbox.attr_k_period, toolbox.attr_d_period, toolbox.attr_take_profit, toolbox.attr_stop_loss), n=1)
...
def evolve_population(population, toolbox, cxpb=CXPB, mutpb=MUTPB, ngen=NGEN):
    ...
    for gen in range(1, ngen + 1):
        ...
        print(f"Gen {gen}: %K Period: {best_ind[0]}, %D Period: {best_ind[1]}, Take-Profit: {best_ind[2]:.2f}%, Stop-Loss: {best_ind[3]:.2f}%")

    return population, logbook

population, logbook = evolve_population(population, toolbox)

best_individual = tools.selBest(population, 1)[0]
k_period, d_period, erc_tp, erc_sl = best_individual
print(f"Best individual for {timeframe} timeframe:\n %K Period: {k_period}, %D Period: {d_period}\n Take-Profit Percentage: {erc_tp:.2f}%\n Stop-Loss Percentage: {erc_sl:.2f}%\n Fitness: {best_individual.fitness.values[0]*100:.2f}%, {best_individual.fitness.values[1]:.2f}")
